...
import { buildJobWorkComparisonRows, exportJobWorkComparisonCsv } from "../../lib/mdOutputReport";
...
/**
 * MD's own Output & Reports dashboard -  mounted only at
 * /md/output/:orderId (see App.tsx). Admin's OutputPage.tsx is a completely
 * separate file/component, deliberately not shared with or refactored out of
 * this one: every chart section below that also appears on admin's page is a
 * verbatim copy, not an extraction, so nothing here can ever change what
 * admin sees.
 *
 * Adds three things admin's page doesn't have: a KPI card row (the
 * procurement chain's own stage-owned figures, plus each production stage's
 * real output), an in-house-vs-job-work production comparison (every entry
 * is already tagged is_job_work -  see migration 024 -  so this is a read,
 * not new tracking), and a denser "Power BI style" panel + chessboard-style
 * matrix tables in place of the plain stage/size tables admin's page shows.
 */
...
/** Checkerboard cell shading -  the "sudoku/chess-board" grid the MD role
 * asked for: alternating both by row AND by column, not just striped rows. */
...
export function MdOutputPage() {
  ...
  // --------------------- In-house vs Outsourced headline ---------------------
  const packingRow = jobWorkRows.find((r) => r.stageKey === STAGE.packing);
  const inHousePacked = packingRow?.inHouse ?? 0;
  const jobWorkPacked = packingRow?.jobWork ?? 0;
  const remainingToOrder = Math.max(chain.totalPcs - inHousePacked - jobWorkPacked, 0);
  const jobWorkTotalAcrossPcsStages = jobWorkRows
    .filter((r) => PCS_PRODUCTION_STAGES.includes(r.stageKey))
    .reduce((s, r) => s + r.jobWork, 0);

  const inHouseDonutData = [
    { name: "In-House", value: inHousePacked },
    { name: "Job Work", value: jobWorkPacked },
    { name: "Remaining", value: remainingToOrder },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-xl font-bold tracking-tight text-ink-900">Production Output - MD Dashboard</h1>
      ...
      {/* <Card>
        <CardHeader
          title="In-House vs Outsourced Production"
          subtitle="Normal data entry = in-house production. Job Work entries = outsourced production. Both are real production_txns rows - this splits them apart, against the total order quantity."
          action={
            <div className="flex gap-2">
              <Button size="sm" variant="secondary" onClick={() => setShowJobWorkModal(true)}>
                View Full Comparison
              </Button>
              <Button size="sm" onClick={() => exportJobWorkComparisonCsv(chain, order)}>
                Download CSV
              </Button>
            </div>
          }
            ...
            <div className="relative mx-auto h-52 w-52">
              ...
              <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
                <p className="text-3xl font-extrabold tabular-nums text-ink-900">
                  {chain.totalPcs > 0 ? `${Math.round(((inHousePacked + jobWorkPacked) / chain.totalPcs) * 100)}%` : "- "}
                </p>
                <p className="text-[11px] font-semibold uppercase tracking-wide text-ink-400">of Order Packed</p>
              </div>
            </div>

            <div className="space-y-3">
              <StatRow dotColor={CHART_BLUE} label="In-House (Packed)" value={inHousePacked} unit="PCS" />
              <StatRow dotColor={CHART_AMBER} label="Job Work (Packed)" value={jobWorkPacked} unit="PCS" />
              <StatRow dotColor={CHART_SLATE} label="Remaining against order" value={remainingToOrder} unit="PCS" />
              <StatRow dotColor={CHART_VIOLET} label="Job Work across Cutting→Packing" value={jobWorkTotalAcrossPcsStages} unit="PCS" />
              <StatRow dotColor={CHART_GREEN} label="Total Order Qty (baseline)" value={chain.totalPcs} unit="PCS" />
            </div>
          </div>
        </CardBody>
      </Card> */}
      ...
      <Modal open={showJobWorkModal} onClose={() => setShowJobWorkModal(false)} title="In-House vs Job Work - Full Comparison" widthClass="max-w-3xl">
        <div className="space-y-4">
            ...
            <table className="w-full min-w-[560px] text-sm">
              <thead>
                <tr className="bg-ink-50 text-[11px] uppercase tracking-wide text-ink-500">
                  ...
                  <th className="px-3 py-2 text-right font-semibold">In-House</th>
                  <th className="px-3 py-2 text-right font-semibold">Job Work</th>
              ...
              <tbody className="divide-y divide-ink-100">
                {jobWorkRows.map((r) => (
                  <tr key={r.stageKey}>
                    <td className="px-3 py-2 font-medium text-ink-900">{r.stageLabel}</td>
                    <td className="px-3 py-2"><Badge tone={r.unit === "KG" ? "neutral" : "brand"}>{r.unit}</Badge></td>
                    <td className="px-3 py-2 text-right font-mono tabular-nums text-blue-700">{r.inHouse.toLocaleString()}</td>
                    <td className="px-3 py-2 text-right font-mono tabular-nums text-amber-700">{r.jobWork.toLocaleString()}</td>
                    <td className="px-3 py-2 text-right font-mono font-semibold tabular-nums">{r.total.toLocaleString()}</td>
                    <td className="px-3 py-2 text-right tabular-nums">{r.pctJobWork != null ? `${r.pctJobWork}%` : "- "}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="flex justify-end gap-2">
            <Button variant="secondary" onClick={() => setShowJobWorkModal(false)}>Close</Button>
            <Button onClick={() => exportJobWorkComparisonCsv(chain, order)}>Download CSV</Button>
          </div>
        </div>
      </Modal>
    </div>
...
function StatRow({ dotColor, label, value, unit }: { dotColor: string; label: string; value: number; unit: string }) {
  return (
