 ...
 * downloads - this file exists only for MdOutputPage.
 *
 * `production_txns.is_job_work` is pure provenance (migration 024) -
 * chain.ts's own arithmetic never reads it, so a stage's real output is
 * always exactly inHouse + jobWork here.
 */
 ...
 * 2 confirmation-only stages (which write neither). */
export function buildJobWorkComparisonRows(chain: ProductionChain): JobWorkComparisonRow[] {
  return chain.stages
    .filter((cs) => cs.txns.length > 0)
    .map((cs) => {
      const inHouse = cs.txns.filter((t) => !t.is_job_work).reduce((sum, t) => sum + (t.qty_out || 0), 0);
      const jobWork = cs.txns.filter((t) => t.is_job_work).reduce((sum, t) => sum + (t.qty_out || 0), 0);
