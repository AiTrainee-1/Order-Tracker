...
const COLLAPSE_KEY = "user-sidebar-collapsed";

export function UserLayout() {
  const { appUser, logout } = useAuth();
  const navigate = useNavigate();
  const [mobileOpen, setMobileOpen] = useState(false);
  // Desktop-only: shrinks the rail to icons so the content area gets more
  // room. Remembered across visits -  mobile ignores this entirely and
  // always shows the full drawer when opened (see the md: guards below).
  ...
  return (
    <div className="min-h-screen font-app" style={spatialBackground}>
      ...
      >
        {/* Desktop-only collapse toggle -  overlaps the rail's edge, same idea
            as VS Code / Notion style sidebars. Hidden on mobile: the drawer
            already has its own open/close affordance (the hamburger button). */}
        <button
        ...
        <div className="shrink-0 border-t border-white/70 px-4 py-4">
          <div
            ...
            title={collapsed ? `${appUser?.name} · @${appUser?.username} · ${appUser?.role}` : undefined}
          >
            <span
