 ...
 * + api/public-order-output.ts) into a QR code, and can bake it - alongside
...
export function ShareQrModal({ open, onClose, order }: ShareQrModalProps) {
  ...
  const shareUrl = `${window.location.origin}/share/output/${order.id}`;
  ...
  async function handleDownload() {
    ...
    try {
      ...
      a.href = url;
      a.download = `${order.io_no}-${order.style}-qr-card.png`.replace(/[^\w.-]+/g, "_");
      a.click();
    ...
    <Modal open={open} onClose={onClose} title="Share QR Code" widthClass="max-w-sm">
      <div className="space-y-4">
        <p className="text-xs text-ink-500">
          Anyone who scans this opens a read-only production dashboard for this order — no login needed.
        </p>
        ...
        <div className="mx-auto flex w-full max-w-[280px] flex-col items-center gap-3 rounded-2xl border border-ink-200 bg-white p-5 text-center shadow-sm">
          ...
          <div>
            ...
            <p className="mt-1 text-sm font-bold text-brand">{order.total_qty.toLocaleString()} PCS</p>
          </div>
...
function drawGarmentGlyph(ctx: CanvasRenderingContext2D, cx: number, cy: number, size: number) {
  ...
  const path = new Path2D(
    "M8.5 3.5 5 6l1.5 2.5L8 7.5V20h8V7.5l1.5 1 1.5-2.5-3.5-2.5c-.5.8-1.5 1.5-3 1.5s-2.5-.7-3-1.5Z",
  );
...
async function buildOrderCardBlob({
  order,
  imageUrl,
  qrDataUrl,
}: {
  ...
  ctx.fillText(`IO ${order.io_no}${order.color ? ` · ${order.color}` : ""}`, centerX, y);
  y += 52;

  const qtyText = `${order.total_qty.toLocaleString()} PCS`;
